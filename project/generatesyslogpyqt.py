...
from PyQt5.QtCore import QTimer, pyqtSignal, QTime, QDate, pyqtSlot
...
import joblib
import os
#from dashandreport import HostTab
...
features = np.hstack((general_tfidf, df[['Event ID']].values))
...
class RealTimeLogViewer(QMainWindow):
    ...
    # Define signals to emit log data
    #log_data_signal = pyqtSignal(str, int, str, str)
    ...
    def __init__(self):
        ...
        self.addlog = HostTab()
        self.addlog.add_log_signal.connect(self.addlog.add_host)
    ...
    def preprocess_log(self, log_entry):
        ...
        features = np.hstack(( general_tfidf, event_id))
        ...
        try:
            for log_type in log_types:
                ...
                for event in events:
                    ...
                    log_features = { 'General': message, 'Event ID': event_id}
                    ...
                    if is_anomalous:
                        ...
                        #self.log_data_signal.emit(source, event_id, message, "Anomaly")
                        #self.addhost.add_host()
                        self.addlog.add_log_signal.emit(source, event_id, message, "Anomaly")
                        
                    else:
                        self.predict_list.addItem("normal")
                        #add prediction for sql
                        # Emit signal with log data
                        #self.addlog.add_log_signal.emit(source, event_id, message, "Normal")
                        #self.addhost.add_host()
