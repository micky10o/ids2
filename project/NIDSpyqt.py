...
import sys 
#from dashandreport import NetTab
...
# List of features 
features_list = [ 'protocol_type', 'service', 'flag', 'src_bytes', 'dst_bytes',
                   'land', 'hot', 
                   'count', 'srv_count',
...
data = data[features_list + ['labels']]
...
class NIDS(QMainWindow):
    ...
    #pkt_summary_signal = pyqtSignal(str)
    #prediction_label_signal = pyqtSignal(str)
    ...
    def __init__(self):
        ...
        self.addnet.add_packet_signal.connect(self.handle_packet_signal)
        self.addnet.add_prediction_signal.connect(self.handle_prediction_signal)
    ...
    def pkt_process(self,pkt):
        ...
        if packet_features is not None:
            ...
            # Emit signals with the captured packet summary and prediction label
           # self.pkt_summary_signal.emit(pkt_summary)
           # self.prediction_label_signal.emit(prediction_label[0])

            #add  sql
            if prediction_label[0] !=  "normal":
                
                 # Emit signals with the captured packet summary and prediction label
                self.addnet.add_packet_signal.emit(pkt_summary)
                self.addnet.add_prediction_signal.emit(prediction_label[0])
                
                #self.addnet.add_network()
            #else:
                #self.addnet.add_packet_signal.emit(pkt_summary)
                #self.addnet.add_prediction_signal.emit(prediction_label[0])
    
    @pyqtSlot(str)
    def handle_packet_signal(self, pkt_summary):
        self.pkt_summary = pkt_summary
        if hasattr(self, 'prediction_label'):
            self.addnet.add_network(self.pkt_summary, self.prediction_label)

    @pyqtSlot(str)
    def handle_prediction_signal(self, prediction_label):
        self.prediction_label = prediction_label
        if hasattr(self, 'pkt_summary'):
            self.addnet.add_network(self.pkt_summary, self.prediction_label)           
